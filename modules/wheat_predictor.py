# -*- coding: utf-8 -*-
"""
麦生育予測モジュール (modules/wheat_predictor.py) - VBA完全一致版
"""
...
WHEAT_VARIETY_PARAMS = {
    "長崎W2号": {
        "a": -227.01,
        "b": 493.65,
        "maturity_sum_temp": 930.0,  # 必要に応じてExcel上のF4の値に合わせてください
    },
    "長崎御島": {
        "a": -209.0,
        "b": 433.84,
        "maturity_sum_temp": 760.0,  # 必要に応じてExcel上のF5の値に合わせてください
    },
    "はるか二条": {
        "a": -223.8,
        "b": 476.16,
        "maturity_sum_temp": 750.0,  # 必要に応じてExcel上のF6の値に合わせてください
    },
}
...
def predict_wheat_growth(
    ...
    """取得済みの気象データを使って麦の出穂日・成熟日を予測する関数（VBA完全一致版）"""
    ...
    # ★VBAに合わせて「常用対数 (底が10)」に変更
    x_val = math.log10(young_spike_length)

    # 調査日から出穂日までに必要な積算気温を算出
    required_temp_to_heading = (p["a"] * x_val) + p["b"]

    current_date = survey_date
    end_date = survey_date + datetime.timedelta(days=150)
    ...
    while current_date <= end_date:
        ...
        if not found_heading:
            ...
            if accumulated_temp >= required_temp_to_heading:
                ...
                found_heading = True
                # VBAのロジックと同様に、出穂日以降の成熟用積算をリセットしてスタート
                accumulated_temp = 0.0
        else:
            accumulated_temp += t
            # 成熟期の積算気温に達したら成熟日
