...
st.set_page_config(
    ...
    page_icon="🌱",
    layout="centered",  # 縦型で見やすいように中央寄せレイアウト
...
if "lat" not in st.session_state or "lon" not in st.session_state:
    ...
    if loc and "coords" in loc:
        ...
        st.session_state["location_label"] = (
            f"現在地 (緯度:{st.session_state['lat']:.4f}, 経度:{st.session_state['lon']:.4f})"
        )
    else:
...
st.text(
    f"現在選択中の地点 ➔ 緯度: {st.session_state['lat']:.4f} / 経度: {st.session_state['lon']:.4f}"
)
...
if run_prediction:
    ...
    with st.spinner("気象データを取得してシミュレーション中..."):
        ...
        if not weather_data:
            result = {
                "error": "❌ 気象データの取得に失敗しました。期間を確認してください。"
            }
        else:
            if crop_category == "水稲":
                ...
                result = predict_onion_downy_mildew(
                    lat, lon, start_dt, weather_data
                )
            elif crop_category == "バレイショ（春作マルチ）":
                ...
                result = predict_scale_insect_peak(
                    lat, lon, target_y, weather_data
                )
            elif crop_category == "レタス（収穫予測）":
    ...
    if "error" in result:
        ...
        if crop_category == "水稲":
            ...
            st.text(
                f"最終積算気温: {result.get('accumulated_temp_at_end', 0):.1f} ℃ (目標: 400.0 ℃)"
            )
            ...
            st.text(
                f"最終積算温度: {result.get('accumulated_temp_at_end', 0):.1f} ℃ (目標: {result.get('target_sum_temp', 0)} ℃)"
            )
            ...
            st.text(
                f"有効積算温度: {result.get('accumulated_gdd', 0):.1f} 日度 (目標: {result.get('target_threshold', 429)} 日度)"
            )
