...
Updated with safe-range weather fetching
@author: user
"""
...
# --- 3. 気象データ取得関連の関数 ---
def fetch_chunk_weather(userid, auth_key, lat, lon, start_date, end_date):
    params = {
        "latitude": str(lat),
        "longitude": str(lon),
    ...
    try:
        ...
        if response.status_code != 200:
            # データの存在しない未来日などの400エラーはサイレント処理または必要に応じてスキップ
            return {}
...
def fetch_real_weather_dict(userid, auth_key, lat, lon, start_date, end_date, chunk_days=60):
    combined_weather = {}
    ...
    while current_start <= end_date:
        ...
        chunk_data = fetch_chunk_weather(userid, auth_key, lat, lon, current_start, current_end)
        if chunk_data:
            combined_weather.update(chunk_data)
        else:
            # データが取れなくなった時点（未来の限界など）以降は取得を打ち切る
            break
...
def run_real_simulation(auth_key):
    ...
    LONGITUDE = 130.0241       
    VARIETY_NAME = "ヒノヒカリ"  
    TRANSPLANT_DATE = datetime.date(2026, 7, 26)  

    p = VARIETY_PARAMS[VARIETY_NAME]
    
    end_date = TRANSPLANT_DATE + datetime.timedelta(days=150) # 水稲の生育期間に合わせ少し短めに設定
